                ...
                if (contacts[update.id]) {
                    ...
                    const contactHashes = await Promise.all(Object.keys(contacts).map(async (contactId) => {
                        if (contactId) {
                            ...
                            return [
                                contactId,
                                (await (0, Utils_1.md5)(Buffer.from(user + "WA_ADD_NOTIF", "utf8")))
                                    .toString("base64")
                                    .slice(0, 3),
                            ];
                        }
                    ...
                    contact =
                        contacts[((_a = contactHashes.find(([, b]) => b === update.id)) === null || _a === void 0 ? void 0 : _a[0]) || ""]; // find contact by attrs.hash, when user is not saved as a contact
                }
                if (contact) {
