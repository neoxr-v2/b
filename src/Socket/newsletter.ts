...
export const makeNewsLetterSocket = (config: SocketConfig) => {
  ...
  const parseFetchedUpdates = async (
    node: BinaryNode,
    ...
    return await Promise.all(
      getAllBinaryNodeChildren(child).map(async (messageNode) => {
        ...
        const reactions = getBinaryNodeChildren(reactionNode, "reaction").map(
          ({ attrs }) =>
            ({ count: +attrs.count, code: attrs.code }) as NewsletterReaction,
        );
  ...
  return {
    ...
    subscribeNewsletterUpdates: async (jid: string) => {
      ...
      return getBinaryNodeChild(result, "live_updates")?.attrs as {
        duration: string;
      };
    },

    newsletterReactionMode: async (
      jid: string,
