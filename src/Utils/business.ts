...
export const parseCollectionsNode = (node: BinaryNode) => {
  ...
  const collections = getBinaryNodeChildren(
    collectionsNode,
    "collection",
  ).map<CatalogCollection>((collectionNode) => {
...
export const parseOrderDetailsNode = (node: BinaryNode) => {
  ...
  const products = getBinaryNodeChildren(
    orderNode,
    "product",
  ).map<OrderProduct>((productNode) => {
...
export async function uploadingNecessaryImagesOfProduct<
  T extends ProductUpdate | ProductCreate,
>(product: T, waUploadToServer: WAMediaUploadFunction, timeoutMs = 30_000) {
  product = {
    ...product,
