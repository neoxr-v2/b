...
describe("Event Buffer Tests", () => {
    ...
    await Promise.all([
      (async () => {
        ...
        ev.emit("chats.upsert", [
          { id: chatId, conversationTimestamp: 123, unreadCount: 1 },
        ]);
      ...
      (async () => {
        ...
        ev.emit("chats.update", [
          { id: chatId, conversationTimestamp: 124, unreadCount: 1 },
        ]);
        const flushed = ev.flush();
        expect(flushed).toBeFalsy();
      })(),
  ...
  it("should buffer message upsert events", async () => {
    ...
    ev.emit("messages.update", [
      { key: msg.key, update: { status: WAMessageStatus.READ } },
    ]);
  ...
  it("should buffer multiple status updates", async () => {
    ...
    ev.emit("messages.update", [
      { key, update: { status: WAMessageStatus.READ } },
    ]);
